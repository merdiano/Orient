...
import com.tps.orientnews.utils.AnimUtils;
...
public class HomeGridItemAnimator extends SlideInItemAnimator {
    ...
    // Pending animations
    private FeedAdapter.OrientNewsHolder pendingAddToPocket;
    private FeedAdapter.OrientNewsHolder pendingStoryCommentsReturn;

    // Currently running animations
    private Pair<FeedAdapter.OrientNewsHolder, AnimatorSet> runningAddToPocket;
    private Pair<FeedAdapter.OrientNewsHolder, AnimatorSet> runningStoryCommentsReturn;
    ...
    public boolean animateChange(@NonNull RecyclerView.ViewHolder oldHolder,
        ...
        if (preInfo instanceof HomeGridItemHolderInfo) {
            ...
            if (info.animateAddToPocket) {
                pendingAddToPocket = (FeedAdapter.OrientNewsHolder) newHolder;
                runPending = true;
            }
            if (info.returnFromComments) {
                pendingStoryCommentsReturn = (FeedAdapter.OrientNewsHolder) newHolder;
                runPending = true;
            }
        }
        return runPending;
    ...
    private void animateAddToPocket(final FeedAdapter.OrientNewsHolder holder) {
        endAnimation(holder);
...
//                ((ViewGroup) holder.pocket.getParent().getParent()).setClipChildren(true);
    ...
    private void animateStoryCommentReturn(final FeedAdapter.OrientNewsHolder holder) {
        endAnimation(holder);
